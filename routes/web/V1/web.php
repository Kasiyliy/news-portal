    ...
    Route::group(['middleware' => ['ROLE_OR:' . \App\Models\Entities\Core\Role::ADMIN_ID]], function () {
        Route::group(['namespace' => 'System', 'prefix' => 'admin'], function () {
            ...
            Route::group(['namespace' => 'Map', 'prefix' => 'map'], function () {
                Route::get('/regions', ['uses' => 'MapController@index', 'as' => 'map.region.index']);
                Route::post('/regions/store-update/{id?}', ['uses' => 'MapController@storeAndUpdate', 'as' => 'map.region.store.update'])->where('id', '[0-9]+');
                Route::get('/regions/create-edit/{id?}', ['uses' => 'MapController@createAndEdit', 'as' => 'map.region.create.edit'])->where('id', '[0-9]+');
                Route::post('/regions/delete/{id}', ['uses' => 'MapController@delete', 'as' => 'map.region.delete'])->where('id', '[0-9]+');
            });
